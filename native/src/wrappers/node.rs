...
fn node_flattened_sign_json(mut cx: FunctionContext) -> JsResult<JsString> {
  let alg = cx.argument::<JsNumber>(0)?;
  let payload = cx.argument::<JsString>(1)?;
  let jwk = cx.argument::<JsString>(2)?;
...
fn node_json_verify(mut cx: FunctionContext) -> JsResult<JsString> {
  let jws = cx.argument::<JsString>(0)?;
  let jwk_string = cx.argument::<JsString>(1)?;

  let jwk: Jwk = serde_json::from_str(&jwk_string.value()).unwrap();
  
  match rust_json_verify(&jws.value(), &jwk) {
    Ok(verified) => {
      let ( payload, _header ) = verified;
      let payload_string = String::from_utf8(payload).unwrap();
      Ok(JsString::new(&mut cx, payload_string))
...
register_module!(mut cx, {
  ...
  cx.export_function("flattened_sign_json", node_flattened_sign_json)?;
  cx.export_function("json_verify", node_json_verify)?;
  cx.export_function("general_sign_json", node_general_sign_json)?;
  Ok(())
});
