    ...
    describe('should encrypt plaintext correctly', () => {
      ...
      describe('where enc type is `GCM`', () => {
        ...
        it("and `enc` = 'A128GCM'", async () => {
          ...
          expect(Buffer.from(encrypted.tag || Uint8Array.from(Buffer.from(""))).toString('base64')).toEqual("aFebG2ev+cSucVzhgvnePw==")
        });
        ...
        it("and `enc` = 'A192GCM'", async () => {
          ...
          expect(Buffer.from(encrypted.tag || Uint8Array.from(Buffer.from(""))).toString('base64')).toEqual("fXf8J7mEhy3Eqxz1mnkwQA==")
        });
        ...
        it("and `enc` = 'A256GCM'", async () => {
          ...
          expect(Buffer.from(encrypted.tag || Uint8Array.from(Buffer.from(""))).toString('base64')).toEqual("no6xZQhgTVtaG6GHuEjlRA==")
        });
      ...
      describe('where enc type is `CBC`', () => {
        ...
        it('and enc=`A128CBC-HS256`', async () => {
          ...
          expect(Buffer.from(encrypted.tag || Uint8Array.from(Buffer.from(""))).toString('base64')).toEqual("8BR2fw5Twj9f7/5S7BEEhw==")
        })
        ...
        it('and enc=`A192CBC-HS384`', async () => {
          ...
          expect(Buffer.from(encrypted.tag || Uint8Array.from(Buffer.from(""))).toString('base64')).toEqual("vylJiL27sZ8Sq63KXNfslNb/aqRhRSyk")
        })
        ...
        it('and enc=`A256CBC-HS512`', async () => {
          ...
          expect(Buffer.from(encrypted.tag || Uint8Array.from(Buffer.from(""))).toString('base64')).toEqual("ZerEUmGJgIblArz+GrdTmP0BNzk2fgqm71OgS6GzzJs=")
        })
