    ...
    describe("should sign payload correctly", () => {
      describe("where didcomm = true", () => {
        it("with a single key", async () => {
          ...
          const signed = await generalSignJson(payload, signer_jwks, true);

          expect(signed.signatures.length).toBe(1);
          expect(signed.signatures[0].header.kid).toBe("did:nuggets:sZziFvdXw8siMvg1P4YS91gG4Lc#key-p256-1");
          expect(signed.signatures[0].protected).toBe(
        ...
        it("with multiple keys of the same `alg` type", async () => {
          ...
          expect(signed.signatures[1].protected).toBe(
            "eyJ0eXAiOiJhcHBsaWNhdGlvbi9kaWRjb21tLXNpZ25lZCtqc29uIiwiYWxnIjoiRVMyNTYifQ"
          );
          expect(signed.payload).toBe("eyJoZWxsbyI6InRoZXJlIn0");
        });
        ...
        it("with multiple keys of different `alg` types", async () => {
          ...
          expect(signed.signatures[1].protected).toBe(
            "eyJ0eXAiOiJhcHBsaWNhdGlvbi9kaWRjb21tLXNpZ25lZCtqc29uIiwiYWxnIjoiRVM1MTIifQ"
          );
          expect(signed.payload).toBe("eyJoZWxsbyI6InRoZXJlIn0");
        });
      });

      describe("where didcomm = false", () => {
        it("with a single key", async () => {
          const alg = "ES256";
          ...
          expect(signed.signatures.length).toBe(1);
          expect(signed.signatures[0].header.kid).toBe("did:nuggets:sZziFvdXw8siMvg1P4YS91gG4Lc#key-p256-1");
          expect(signed.signatures[0].protected).toBe("eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9");
          expect(signed.payload).toBe("eyJoZWxsbyI6InRoZXJlIn0");
        });
        ...
        it("with multiple keys of the same `alg` type", async () => {
          ...
          expect(signed.signatures[0].protected).toBe("eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9");
          expect(signed.signatures[1].header.kid).toBe("did:nuggets:qy8tyYBwveRXKDL2jjYTZENBDi3#key-p256-1");
          expect(signed.signatures[1].protected).toBe("eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9");
          expect(signed.payload).toBe("eyJoZWxsbyI6InRoZXJlIn0");
        });
        ...
        it("with multiple keys of different `alg` types", async () => {
          ...
          expect(signed.signatures[0].protected).toBe("eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9");
          expect(signed.signatures[1].header.kid).toBe("did:nuggets:qy8tyYBwveRXKDL2jjYTZENBDi3#key-p512-1");
          expect(signed.signatures[1].protected).toBe("eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzUxMiJ9");
          expect(signed.payload).toBe("eyJoZWxsbyI6InRoZXJlIn0");
        });
      });
    });
