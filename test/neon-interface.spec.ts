...
import type { JoseModule } from "../src/types/index.js";
import jwks from "../__tests__/jwks.json";
  ...
  describe("Functions", () => {
    ...
    describe("compact_sign_json", () => {
      describe("should sign json with private key", () => {
        it("where didcomm=true", () => {
          ...
          expect(Buffer.from(header_b64, "base64").toString()).toBe(
            '{"typ":"application/didcomm-signed+json","alg":"ES256","kid":"did:nuggets:sZziFvdXw8siMvg1P4YS91gG4Lc#key-p256-1"}'
          );
        ...
        it("where didcomm=false", () => {
          ...
          expect(Buffer.from(header_b64, "base64").toString()).toBe(
            '{"typ":"JWT","alg":"ES256","kid":"did:nuggets:sZziFvdXw8siMvg1P4YS91gG4Lc#key-p256-1"}'
          );
          expect(Buffer.from(payload_b64, "base64").toString()).toBe(payload);
        });
      });
    ...
    describe("compact_json_verify", () => {
      it("should verify signed json", () => {
        ...
        const jws =
          "eyJ0eXAiOiJhcHBsaWNhdGlvbi9kaWRjb21tLXNpZ25lZCtqc29uIiwiYWxnIjoiRVMyNTYiLCJraWQiOiJkaWQ6bnVnZ2V0czpzWnppRnZkWHc4c2lNdmcxUDRZUzkxZ0c0TGMja2V5LXAyNTYtMSJ9.eyJoZWxsbyI6InRoZXJlIn0.TtXvFTkQvD9SOve5yzEgzLAVgAm9WefaP99A0HZrYztniE2GD9HLNqVlf2b1VzCvDtvy4Iq54UUtX8079pPSOg";
      ...
      describe("should throw", () => {
        it("where public key is incorrect", () => {
          ...
          expect(() => jose.compact_json_verify(jws_valid, jwk_public_incorrect)).toThrow(
            /internal error in Neon module: Failed to verify data/
          );
        });
        ...
        it("where payload has been changed", () => {
          ...
          const jws_invalid =
            "eyJ0eXAiOiJhcHBsaWNhdGlvbi9kaWRjb21tLXNpZ25lZCtqc29uIiwiYWxnIjoiRVMyNTYiLCJraWQiOiJkaWQ6bnVnZ2V0czpzWnppRnZkWHc4c2lNdmcxUDRZUzkxZ0c0TGMja2V5LXAyNTYtMSJ9.eyJoZWxsbyI6InlvdSJ9.TtXvFTkQvD9SOve5yzEgzLAVgAm9WefaP99A0HZrYztniE2GD9HLNqVlf2b1VzCvDtvy4Iq54UUtX8079pPSOg";

          expect(() => jose.compact_json_verify(jws_invalid, jwk_public)).toThrow(
            /internal error in Neon module: Failed to verify data/
          );
        });
      });
    ...
    describe("flattened_sign_json", () => {
      describe("should sign json with private key", () => {
        it("where didcomm=true", () => {
          ...
          expect(Buffer.from(jws.protected, "base64").toString()).toBe(
            '{"typ":"application/didcomm-signed+json","alg":"ES256"}'
          );
    ...
    describe("json_verify", () => {
      describe("should verify signed json", () => {
        it('where structure is "flattened" (only for single signatures)', () => {
          ...
          const jws =
            '{"protected":"eyJ0eXAiOiJhcHBsaWNhdGlvbi9kaWRjb21tLXNpZ25lZCtqc29uIiwiYWxnIjoiRVMyNTYifQ","header":{"kid":"did:nuggets:sZziFvdXw8siMvg1P4YS91gG4Lc#key-p256-1"},"payload":"eyJoZWxsbyI6InRoZXJlIn0","signature":"y_Ytj-OJmLfnR_LUcFD60KZJuiqfWJBrATdvvVg3bn1pS5fpaKCNb3JBv1EAZ7gz7NrI_MoG3_xfSjBALRt1-w"}';
      ...
      describe("should throw", () => {
        it("where public key is incorrect", () => {
          ...
          expect(() => jose.json_verify(jws_valid, jwk_public_incorrect)).toThrow(
            /internal error in Neon module: Failed to verify data/
          );
        });
        ...
        it("where payload has been changed", () => {
          ...
          const jws_invalid =
            '{"protected":"eyJ0eXAiOiJhcHBsaWNhdGlvbi9kaWRjb21tLXNpZ25lZCtqc29uIiwiYWxnIjoiRVMyNTYifQ","header":{"kid":"did:nuggets:sZziFvdXw8siMvg1P4YS91gG4Lc#key-p256-1"},"payload":"eyJoZWxsbyI6InlvdSJ9","signature":"y_Ytj-OJmLfnR_LUcFD60KZJuiqfWJBrATdvvVg3bn1pS5fpaKCNb3JBv1EAZ7gz7NrI_MoG3_xfSjBALRt1-w"}';

          expect(() => jose.json_verify(jws_invalid, jwk_public)).toThrow(
            /internal error in Neon module: Failed to verify data/
          );
        });
      });
    ...
    describe("general_sign_json", () => {
      ...
      it("should sign json with private keys of the same `alg` type", () => {
        const jwt = { hello: "there" };
        const payload = JSON.stringify(jwt);
        const signer_jwks = JSON.stringify([
          { ...jwks[0].private, alg: "ES256" },
          { ...jwks[1].private, alg: "ES256" },
        ]);
